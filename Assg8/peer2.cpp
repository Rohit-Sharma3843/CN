...
#define PORT_LISTEN 54001
#define PORT_CONNECT 54000
...
void receive_thread_func(int sock) {
    ...
    while (true) {
        ...
        if (r <= 0) {
            cerr << "Peer2: read error or connection closed\n";
            break;
        }
        buffer[r] = '\0';
        cout << "\nreceiver : " << buffer << "\nYou: " << flush;
...
int main() {
    ...
    if (inet_pton(AF_INET, IP, &addr_listen.sin_addr) <= 0) {
        perror("Peer2: inet_pton listen failed");
        return 1;
    }

    if (bind(listen_fd, (sockaddr*)&addr_listen, sizeof(addr_listen)) < 0) {
        perror("Peer2: bind() failed");
        return 1;
    }

    if (listen(listen_fd, 1) < 0) {
        perror("Peer2: listen() failed");
        return 1;
    }

    cout << "Peer2 listening on " << IP << ":" << PORT_LISTEN << endl;
    ...
    while (true) {
        ...
        if (sock_to_peer < 0) {
            perror("Peer2: socket to peer failed");
            return 1;
        }
        ...
        if (inet_pton(AF_INET, IP, &peer_addr.sin_addr) <= 0) {
            perror("Peer2: inet_pton peer failed");
            close(sock_to_peer);
            return 1;
        }

        cout << "Peer2: trying to connect to Peer1 at " << IP << ":" << PORT_CONNECT << "\n";
        if (connect(sock_to_peer, (sockaddr*)&peer_addr, sizeof(peer_addr)) == 0) {
            cout << "Peer2: connected to Peer1\n";
            break;
        }
        perror("Peer2: connect failed");
        close(sock_to_peer);
        sleep(1);
    }
    ...
    if (sock_from_peer < 0) {
        perror("Peer2: accept failed");
        return 1;
    }
    cout << "Peer2: accepted connection from Peer1\n";
