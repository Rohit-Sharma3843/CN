...
public class RohitSharma_7347_assg5 {
    ...
    public static String intToBinaryIP(int ip) {
        return toBinary((ip >>> 24) & 0xff) + " " + toBinary((ip >>> 16) & 0xff) + " " + toBinary((ip >>> 8) & 0xff)
                + " " + toBinary(ip & 0xff);
    }
    ...
    public static void calculateSubnetDetails(String ip, int prefix, int requiredSubnets) {
        ...
        int ipNum = (Integer.parseInt(parts[0]) << 24) | (Integer.parseInt(parts[1]) << 16)
        ...
        for (int i = 0; i < requiredSubnets; i++) {
            ...
            System.out.println("\nSubnet " + (i + 1) + ":");
            System.out.println(
                    "  Network Address: " + intToIP(networkAddress) + " (" + intToBinaryIP(networkAddress) + ")");
            System.out.println("  First Usable Host: " + intToIP(firstHost) + " (" + intToBinaryIP(firstHost) + ")");
            System.out.println("  Last Usable Host: " + intToIP(lastHost) + " (" + intToBinaryIP(lastHost) + ")");
            System.out.println(
                    "  Broadcast Address: " + intToIP(broadcastAddress) + " (" + intToBinaryIP(broadcastAddress) + ")");
        }
    }
